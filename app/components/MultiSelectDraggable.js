...
function MultiSelect({ options, label, selectedOptions, onSelect }) {
  const [searchTerm, setSearchTerm] = useState('');
  const [showOptions, setShowOptions] = useState(false);
  const dragItemIndex = useRef(null); // Armazena o índice do item que está sendo arrastado
  const dragOverItemIndex = useRef(null); // Armazena o índice do item sobre o qual estamos passando
  ...
  // Função para trocar a ordem dos itens arrastados
  const handleDrop = () => {
    const copiedOptions = [...selectedOptions];
    const draggedItem = copiedOptions.splice(dragItemIndex.current, 1)[0]; // Remove o item arrastado
    copiedOptions.splice(dragOverItemIndex.current, 0, draggedItem); // Reinsere na nova posição

    onSelect(copiedOptions); // Atualiza a lista reordenada
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {selectedOptions.map((option, index) => (
          <div
            ...
            draggable // Habilita o item para ser arrastado
            onDragStart={() => (dragItemIndex.current = index)} // Armazena o índice do item sendo arrastado
            onDragOver={(e) => {
              e.preventDefault();
              dragOverItemIndex.current = index; // Armazena o índice do item sobre o qual estamos passando
            }}
            onDrop={handleDrop} // Quando o item é solto, realiza a troca
          >

...
const TripTable = ({columns,data,onEdit,onDelete, onExport,onClone,itemsPerPage = 10, permissions }) => {
      ...
      <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
          <tr>
            ...
            {(permissions.edit || permissions.delete || permissions.export || permissions.clone) && (
        ...
        <tbody>
          {currentData.length > 0 ? (
              ...
              >
                ...
                {columns.map((column) => (
                  <td key={column.key} className="px-6 py-4">
                    {column.key === 'startDate' || column.key === 'endDate' ? (
                      new Date(row[column.key]).toLocaleDateString() // Formatação da data
                ...
                {(permissions.edit || permissions.delete || permissions.export || permissions.clone) && (
                  <td className="px-6 py-4 inline-block px-2 py-1 text-sm">
                    ...
                    {permissions.clone && (
                      <button
                        onClick={() => onClone(row._id || row.id)}
                        className="font-medium text-blue-600 dark:text-blue-500 hover:underline mr-2"
                      >
                        Clone
                      </button>
                    ...
                    {permissions.export && (
                      <button
                        onClick={() => onExport(row._id || row.id)}
                        className="font-medium text-blue-600 dark:text-blue-500 hover:underline mr-2"
                      >
                        Export
                      </button>
            ...
            <tr>
              <td colSpan={columns.length + 3} className="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                No Records.
              </td>
            </tr>
          )}
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-1/3">
            ...
            <div className="flex justify-end space-x-2">
              <button
                ...
                className="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-gray-300 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600"
              >
                Cancel
              </button>
              <button

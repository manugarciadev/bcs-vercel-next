...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    ...
    try {
      ...
      const response = await axios.post('https://bukingmaster.com/api_/login', { email, password });
      // Verifique a resposta e envie o token ou outra informação ao cliente
      const { token, refreshToken, user } = response.data; // Supondo que o backend retorne um token JWT
